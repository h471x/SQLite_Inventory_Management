...
void ProductInit::tableProduct(){
    QSqlQuery query;
    query.exec("CREATE TABLE IF NOT EXISTS MATERIEL (IdMateriel INTEGER NOT NULL UNIQUE, NomMateriel TEXT NOT NULL , Marque TEXT NOT NULL ,NomEtat TEXT NOT NULL ,DEnregistrement TEXT NOT NULL , Emprunter INTEGER NOT NULL DEFAULT 0 CHECK (Emprunter IN (0,1)),IdCategorie INTEGER NOT NULL ,NomFournisseur TEXT  ,UsernameAdmin TEXT, UsernameUtilisateur TEXT,PRIMARY KEY(IdMateriel),FOREIGN KEY(IdCategorie)REFERENCES CATEGORIE (IdCategorie) ON UPDATE CASCADE ON DELETE CASCADE FOREIGN KEY(NomFournisseur) REFERENCES FOURNISSEUR (NomFournisseur) ON UPDATE SET NULL ON DELETE SET NULL FOREIGN KEY(UsernameAdmin) REFERENCES ADMIN (UsernameAdmin) ON UPDATE CASCADE ON DELETE SET NULL FOREIGN KEY (UsernameUtilisateur) REFERENCES UTILISATEUR (UsernameUtilisateur) ON UPDATE SET NULL ON DELETE SET NULL FOREIGN KEY (NomEtat) REFERENCES ETAT (NomEtat) ON UPDATE SET NULL ON DELETE SET NULL);");
}
